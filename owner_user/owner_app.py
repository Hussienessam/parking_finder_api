...
def create(db):
    try:
        ...
        validated, errors = validator.validate(request.json, validator.create_garage_schema)
        if validated:
...
def update(db):
    try:
        garage_ref = db.collection('Garages')
        validated, errors = validator.validate(request.json, validator.update_garage_schema)
        if validated:
            id = request.json['id']
